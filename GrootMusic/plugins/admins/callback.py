...
from GrootMusic.utils.formatters import seconds_to_min
from GrootMusic.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from GrootMusic.utils.stream.autoclear import auto_clean
from GrootMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Groot.resume_stream(chat_id)
        ...
        await Groot.mute_stream(chat_id)
        ...
        await Groot.unmute_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Groot.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Groot.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Groot.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            ...
            try:
                await Groot.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Groot.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
